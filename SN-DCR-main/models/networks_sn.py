...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'resnet_9blocks':
        ...
        net = RResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
    elif netG == 'unet_256':
...
class MuSELayer(nn.Module):
    def __init__(self, channel, reduction=16):
        super(MuSELayer, self).__init__()
        self.h_avg_pool = nn.AdaptiveAvgPool2d((1, None))
        self.w_avg_pool = nn.AdaptiveAvgPool2d((None, 1))
        
        self.h_max_pool = nn.AdaptiveMaxPool2d((1, None))
        self.w_max_pool = nn.AdaptiveMaxPool2d((None, 1))
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        y_h1 = self.h_avg_pool(x)
        y_w1= self.w_avg_pool(x)

        y_h2 = self.h_max_pool(x)
        y_w2 = self.w_max_pool(x)

        y1=y_w1*y_h1
        y11=self.avg_pool(y1).view(b, c)
        y2=y_w2*y_h2
        y22=self.avg_pool(y2).view(b, c)
        yh=y11+y22

        y = self.fc(yh).view(b, c, 1, 1)
...
class RResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        """
        assert(n_blocks >= 0)
        super(RResnetGenerator, self).__init__()
        self.opt = opt
        ...
        self.model1 = nn.Sequential(nn.ReflectionPad2d(3),
                 nn.Conv2d(input_nc, 64, kernel_size=7, padding=0, bias=use_bias),
                 norm_layer(64),
                 nn.ReLU(True),
                 FcaLayer(64, 8, 32, 32))
        #model += [MuSELayer(64)]
        ...
        self.model2 = nn.Sequential(nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          norm_layer(128),
                          nn.ReLU(True),
                          Downsample(128),
        ...
        self.model3 =nn.Sequential (nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          norm_layer(256),
                          nn.ReLU(True),
                          Downsample(256),
        ...
        smodel = [ResnetBlock(256, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        for i in range(n_blocks-1):       # add ResNet blocks

            smodel += [ResnetBlock(256, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        
        self.resmodel=nn.Sequential(*smodel)
                                        
      
        self.model4=nn.Sequential(Upsample(256),          # add upsampling layers
                nn.Conv2d(256, int(128),
                ...
                nn.ReLU(True),
                FcaLayer(128, 8, 32, 32))
        self.model5= nn.Sequential(Upsample(128),          # add upsampling layers
                nn.Conv2d(128, int(64),
                                    kernel_size=3, stride=1,
                                    padding=1,  # output_padding=1,
                                    bias=use_bias),
                norm_layer(int(64)),
        ...
        self.model6 = nn.Sequential(nn.ReflectionPad2d(3),
        ...
        #model = self.model1+self.model2+self.model3+self.resmodel+self.model4+self.model5+self.model6
        self.model = nn.Sequential(*(self.model1,self.model2,self.model3,self.resmodel,self.model4,self.model5,self.model6))
    ...
    def forward(self, input, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            ...
            c1_de = self.model4(skip1_de)

        
            skip2_de = torch.cat((c2, c1_de), 1)

        
            c3_de = self.model5(skip2_de)

        
            skip3_de = torch.cat((c1, c3_de), 1)

        
            c4_de = self.model6(skip3_de)
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                ...
                model += [Upsample(ngf * mult),
                          ...
                          norm_layer(int(ngf * mult / 2)),
                          nn.ReLU(True),
                          FcaLayer(int(ngf * mult / 2), 8, 32, 32)
                          ]
        model += [nn.ReflectionPad2d(3)]
...
class sSCConv(nn.Module):
    def __init__(self, planes,  pooling_r):
        super(sSCConv, self).__init__()
        self.k2 = nn.Sequential(
                    nn.AvgPool2d(kernel_size=pooling_r, stride=pooling_r), 
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [SNConv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim), nn.ReLU(True)]
        #conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim), nn.ReLU(True),FcaLayer(dim, 8, 32, 32)]
        ...
        #conv_block += [SNConv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim)]
        #conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim),SCBottleneck(dim,dim)]
...
class style_Loss:
    ...
    def __call__(self, x, y_hat):
        ...
        #x= tfs.ToTensor()(x)
        #y_hat= tfs.ToTensor()(y_hat)
        # 将图片变成网络输出的维度
        #x = Variable(torch.unsqueeze(x, dim=0).float(), requires_grad=False)
        #y_hat = Variable(torch.unsqueeze(y_hat, dim=0).float(), requires_grad=False)
        #style_features = self.vgg(x)
        #y_hat_features = self.vgg(y_hat)

        style_gram =[gram(fmap) for fmap in x]
        y_hat_gram = [gram(fmap) for fmap in y_hat]
        L_style = 0
        for j in range(2):
